...
def test_folder_creation(api_client, authenticated_user):
    """Тест создания папки"""
    ...
    assert Folder.objects.filter(name='New Folder', owner=authenticated_user).exists()


def test_file_upload(api_client, authenticated_user):
    """Тест загрузки файла"""
    ...
    assert File.objects.filter(original_name="test.txt", owner=authenticated_user).exists(), \
        "File record was not created in database"


def test_file_upload_invalid_extension(api_client, authenticated_user):
...
def test_file_list(api_client, authenticated_user, folder, file):
    """Тест получения списка файлов"""
    api_client.force_authenticate(user=authenticated_user)
    url = reverse('cloud:file-list') + f"?folder={folder.id}"
    response = api_client.get(url)
    assert response.status_code == status.HTTP_200_OK
    assert len(response.data) > 0


def test_file_download(api_client, authenticated_user, file):
    """Тест скачивания файла"""
    api_client.force_authenticate(user=authenticated_user)
    url = reverse('cloud:file-download', kwargs={'pk': file.id})
    response = api_client.get(url)
    assert response.status_code == status.HTTP_200_OK
    assert response['Content-Disposition'] == f'attachment; filename="{file.original_name}"'


def test_file_download_unauthorized(api_client, file):
    """Тест скачивания файла не аутентифицированным пользователем"""
    url = reverse('cloud-v1:file-download', kwargs={'pk': file.id})
    response = api_client.get(url)
    assert response.status_code == status.HTTP_403_FORBIDDEN


def test_file_delete(api_client, authenticated_user, file):
    """Тест удаления файла."""
    api_client.force_authenticate(user=authenticated_user)
    url = reverse('cloud:file-delete', kwargs={'pk': file.id})
    response = api_client.delete(url)
    assert response.status_code == status.HTTP_204_NO_CONTENT
    assert not File.objects.filter(id=file.id).exists()


def test_file_rename(api_client, authenticated_user, file):
    """Тест переименования файла"""
    api_client.force_authenticate(user=authenticated_user)
    url = reverse('cloud:file-rename', kwargs={'pk': file.id})
    data = {'new_name': 'renamed.txt'}
    response = api_client.patch(url, data, format='json')

    assert response.status_code == status.HTTP_200_OK
    file.refresh_from_db()
    assert file.original_name == 'renamed.txt'


def test_file_update_comment(api_client, authenticated_user, file):
    """Тест обновления комментария к файлу"""
    api_client.force_authenticate(user=authenticated_user)
    url = reverse('cloud:file-update-comment', kwargs={'pk': file.id})
    data = {'comment': 'New comment'}
    response = api_client.patch(url, data, format='json')
    assert response.status_code == status.HTTP_200_OK
    file.refresh_from_db()
    assert file.comment == 'New comment'


def test_file_public_link(api_client, authenticated_user, file):
    """Тест создания публичной ссылки на файл"""
    api_client.force_authenticate(user=authenticated_user)
    url = reverse('cloud:file-public-link', kwargs={'pk': file.id})
    ...
    assert file.public_link is not None
    assert isinstance(file.public_link, uuid.UUID)


def test_file_download_by_link(api_client, file):
    """Тест скачивания файла по публичной ссылке"""
    file.public_link = uuid.uuid4()
    file.save()

    url = reverse('cloud:file-download-by-link', kwargs={'public_link': file.public_link})
    response = api_client.get(url)
